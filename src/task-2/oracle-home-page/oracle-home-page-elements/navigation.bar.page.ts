...
export class NavigationBarPage {
     ...
     */
    public constructor(webdriver: WebDriver, navigationBarModel: NavigationBarModel) {
    ...
    public async selectResourcesFromNavMenu(): Promise<void> {
        const resourcesButton: WebElement = await this.driver.findElement(
    ...
    public async selectJavaDownloadLink(): Promise<void> {
        const javaDownloadLink: WebElement = await this.driver.findElement(
            this.navigationMenuModel.resourcesSubMenu.JAVA_DOWNLOAD_LINK);
        await this.driver.wait(until.elementIsVisible(javaDownloadLink), 2000);
